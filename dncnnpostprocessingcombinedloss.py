...
output_dir = '/mmfs1/gscratch/uwb/vdhaya/output350combinedloss'
...
# Train the model
model.fit(X_train, y_train, epochs=350, batch_size=8, verbose=1)
...
for i, image_path in enumerate(sorted(glob(os.path.join(noisy_dir, '*.flt')))):
    ...
    output_image_path = os.path.join(output_dir, f'denoised_image_{i+1:04}.flt')
